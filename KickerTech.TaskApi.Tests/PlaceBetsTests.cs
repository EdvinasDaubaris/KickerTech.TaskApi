...
namespace KickerTech.TaskApi.Tests
{
    public class PlaceBetsTests
    {
        ...
        public async Task HandleAsync_ShouldReturnSuccess_WhenRequestIsValid()
        {
            ...
            A.CallTo(() => _betsServiceFake.CreateBet(A<int>._, A<int>._, A<int>._, A<decimal>._, A<ResultCode>._)).DoesNothing();

            // Act
            await _endpoint.HandleAsync(request, CancellationToken.None);

            // Assert
            A.CallTo(() => _betsServiceFake.CreateBet(1, 1, 1, 50, ResultCode.Success)).MustHaveHappenedOnceExactly();
        ...
        public async Task HandleAsync_ShouldReturnFailed_WhenPlayerHasInsufficientBalance()
        {
            ...
            // Assert
            A.CallTo(() => _betsServiceFake.CreateBet(1, 1, 1, 50, ResultCode.Failed)).MustHaveHappenedOnceExactly();
        ...
        public async Task HandleAsync_ShouldReturnFailed_WhenBetSumIsInvalid()
        {
            ...
            // Assert
            A.CallTo(() => _betsServiceFake.CreateBet(A<int>._, A<int>._, A<int>._, A<decimal>._, ResultCode.Failed)).MustHaveHappenedOnceExactly();
            A.CallTo(() => _playersServiceFake.UpdatePlayerBalance(A<int>._, A<decimal>._)).MustNotHaveHappened();
        }
    }
}
